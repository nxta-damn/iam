...
class ChangePolicyDescription(Command[None]):
    policy_identity: PolicyIdentity
    description: str


class ChangePolicyDescriptionHandler(CommandHandler[ChangePolicyDescription, None]):
    def __init__(
        self,
    ...
    def handle(self, command: ChangePolicyDescription) -> None:
        ...
        is_authorized = self._policy_enforcement_point.authorize(
            action=AuthorizationAttribute(
                target_name="action", attributes={"name": "change_policy_description"}
            ),
