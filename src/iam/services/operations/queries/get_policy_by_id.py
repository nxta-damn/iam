...
class GetAbacPolicy(Query[AbacPolicyReadModel]):
    policy_id: PolicyIdentity


class GetAbacPolicyHandler(QueryHandler[GetAbacPolicy, AbacPolicyReadModel]):
    def __init__(
        self,
    ...
    def handle(self, query: GetAbacPolicy) -> AbacPolicyReadModel:
        ...
        is_authorized = self._policy_enforcement_point.authorize(
            action=AuthorizationAttribute(target_name="action", attributes={"name": "get_policy_by_id"}),
        ...
        if not is_authorized:
            raise ApplicationError(
                message="User is not authorized to get abac policy", error_type=ErrorType.UNAUTHORIZED
            )

        abac_policy = self._policy_gateway.with_id(policy_id=query.policy_id)

        if not abac_policy:
            raise ApplicationError(
                message=f"Abac policy with id: {query.policy_id} is not found", error_type=ErrorType.NOT_FOUND
            )
